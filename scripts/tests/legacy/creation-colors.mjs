export function swatchOwners(creation, id) {
  return creation.objects.filter(
    (o) =>
      ...
      o.surfaceGraph?.outputs.some((o) => o.style?.swatchId === id) ||
...
export function removeSwatch(project, creation, id, replacementId) {
  ...
  for (const o of owners) {
    ...
    for (const output of o.surfaceGraph?.outputs || [])
      if (output.style?.swatchId === id) output.style.swatchId = replacement.id;
