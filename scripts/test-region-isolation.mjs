...
for (const [action, args] of [
  ...
  assert.deepEqual(
    next.cells.filter((c) => c.key !== target.key),
    scene.cells.filter((c) => c.key !== target.key),
  );
  assert.deepEqual(
...
const compiled = compileCreation(edited);
assert.deepEqual(
  compiled.model.features.filter((f) => object.featureIds.includes(f.id)),
  project.model.features.filter((f) => object.featureIds.includes(f.id)),
);
console.log(
