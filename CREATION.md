...
5. 在“制作 → 打印分层”设置打印层高，建立堆叠层并分配部件；厚度按整数打印层设置，上层跟随下层最高点。“生成承托部件”是可选操作，有完整底层轮廓时无需另加。
...
选择工具只选择，拖过面或线条都不移动形状。按 A 进入节点工具后修改当前路径；Shift / Ctrl 点击可增减节点，Ctrl+A 全选当前路径节点，拖动已选节点一起移动。空格拖动或中键拖动始终平移画布，包括面覆盖区和高倍缩放。节点拖动中的 Esc 恢复原位置，松手后 Ctrl+Z 一步撤销。

多选区域后，普通单击其中一块收为单选；画布 Shift / Ctrl 点击增减区域，只有作品树的 Shift 按列表连续选择。V 的空白拖动只框选线条，不移动几何；V 下 L、Delete 不改形状。选择工具的线条命中范围贴近实际线宽，方便点击杯沿等窄区域；节点工具仍保留较宽的线条命中范围。三维重叠面按实际深度与共面显示顺序选择，平移、旋转不会改变选区。
...
| 部件内部   | 线条和局部区域收在部件下，区域直接显示名称和颜色提示               |
...
| 制作       | 打印分层、可选承托部件、实体检查、导出；高级构造和制造参数折叠收纳 |
...
新建分区时，子区域继承原来的颜色和厚度。日常编辑按构造输出身份保留样式，不按面积重新猜匹配。源线修改破坏既有分区或输出引用时，暂停受影响的对象输出链；修复源线可自动恢复，结构变更须明确重建或解除步骤。不再生成冲突条纹要求重新填色。

区域形状每次都从当前样条和构造关系重新计算。部件中的“挖洞”同时作用于原有体块、普通平面和新建分区；旧工程不需要先转换或重新填色。预览、选择和导出使用扣洞后的区域，挖洞不会改写其他部件共用的来源配方。孔完全覆盖区域时允许结果为空。

旧工程的填色范围仅用于一次性迁移绑定；绑定后以来源和输出身份维护样式。无效边界、未闭合挖洞或丢失输出会明确报错，失败部件停止产出，保留源线与样式以便修正或撤销；修正后自动重算，不回退到旧几何。
...
- **通用 3MF**：默认打印导出，保留分色实体、毫米尺寸和位置，不绑定打印机。Bambu 工程模板导出是独立可选入口；STL 保留兼容 API。
- **Blender**：Python 脚本生成最终实体和独立隐藏集合中的精确源贝塞尔。成品实体为单一材质；项目颜色保存在工程和分色 SVG 中。脚本在 Blender 的 Scripting 工作区运行。

立体视图是带颜色的分区挤出预览，用来编辑外观；最终实体另外执行三维布尔和制造检查。高级切削效果以实际实体检查 / 导出为准。当前支持分色 3MF；不提供圆角、斜面或自动最小壁厚分析。
...
以下命令保留兼容入口；`continue_partition` 已退出日常流程，新分区进入修改器栈。`creation_command` 还支持 `new_object`、`object`、`swatch`、`move_paths`、`roles`、`reorder`、`continue_partition`、`combine_objects`。`roles` 参数为 `{objectId,pathIds,role}`，用途为 `boundary / divider / hole / guide`；`move_paths` 使用 `{pathIds,objectId}`；`object` 使用 `{id,changes}`。完整工具描述见 `lib/creation-api.ts`。
