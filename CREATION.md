...
新建分区时，子区域继承原来的颜色和厚度。保持分区结构的拖线编辑按一对一区域对应保留样式；若合并了不同颜色或高度，合并区域显示冲突条纹，必须选择保留的样式才能导出。独立闭合边界的新填色同时记录来源身份，边界移出原填色范围后仍能保留颜色和高度。

区域形状每次都从当前样条和构造关系重新计算。部件中的“挖洞”同时作用于原有体块、普通平面和新建分区；旧工程不需要先转换或重新填色。预览、选择和导出使用扣洞后的区域，挖洞不会改写其他部件共用的来源配方。孔完全覆盖区域时允许结果为空。

保存的填色范围只用于样式匹配，不作为区域形状的回退来源。删除边界后不会留下旧色块；没有对应区域的样式记录不产生面，也不会单独阻止导出。无效边界或未闭合的挖洞路径会明确报错，失败部件停止显示区域，保留源线和样式以便修正或撤销；修正后自动重算。
