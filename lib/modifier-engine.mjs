...
const valid = (g) => {
  ...
  return g.isEmpty() || g.getArea() < 1e-7 ? null : g;
};
...
export function evaluateSourceProgram(project, feature, object, ctx) {
  const source = object.sources?.[feature.id];
  let g = ctx.get(source?.regionId || feature.regionId);
  for (const m of source?.modifiers || []) {
    if (!m.enabled) continue;
    try {
      g = g[m.operation](regionInput(project, m.input, ctx));
    } catch (e) {
      throw Error(`来源「${m.name}」：${e.message}`);
    }
  }
  return g;
}
...
function splitGeometry(project, geometry, modifier) {
  ...
  const result = pieces.map((g) => ({ g, side: side(g) }));
...
export function applyObjectModifiers(project, scene) {
    ...
    try {
      for (const modifier of object.modifiers || []) {
        ...
        try {
          ...
          throw Error(`修改器「${modifier.name}」：${e.message}`);
        }
      }
      done.set(id, cells);
      return cells;
  ...
  for (const o of scene.creation.objects) {
    try {
      ...
      if (!errors.some((error) => error.objectId === o.id))
        errors.push({ objectId: o.id, kind: 'modifier', message: e.message });
    }
  }
  ...
  const overrides = { ...scene.holeFeatureOverrides };
  ...
  return {
    ...
    holeFeatureOverrides: overrides,
