...
function candidates(project, o, joinMM = 0) {
  ...
  if (!boundaries.length && !o.baseRegionIds?.length && !o.basePathIds?.length)
  ...
  if (!areas.length) return { cells: [], connections: [] };
  ...
  for (let i = 0; i < open.length; i++) {
    ...
    for (const end of [0, -1]) {
      ...
      if (gap > 1e-7 && gap <= joinMM) {
        connections.push({ from, to, gapMM: gap });
...
export function evaluateCreation(project, options = {}) {
    ...
    try {
      ...
      for (let i = 0; i < result.cells.length; i++) {
        ...
        const signatures = new Set(
          overlaps.map((h) => h.p.swatchId + ':' + h.p.heightMM),
        );
  ...
  return { creation, cells, errors, connections, topologies };
}
export function compileCreation(project) {
  ...
  for (const o of scene.creation.objects)
    for (const f of p.model.features) {
      if (o.replacedFeatureIds?.includes(f.id)) f.enabled = false;
