    ...
    try {
      const z = zFor(o),
        ...
        automaticLegacyPartition =
          (hasPartitionRole(o) || o.legacyPartition === true) &&
          !o.replacedFeatureIds?.length,
        result = candidates(
          project,
          o,
      ...
      // Do not leave a half-built candidate set beside removed legacy cells.
      // Saved paint footprints are the last known good object topology and
      // remain visible for recovery, while the retained error blocks export.
      ...
      cells.push(...legacy);
      if (o.paints.length)
        for (let i = 0; i < o.paints.length; i++) {
          const paint = o.paints[i],
            geometry = scaleGeometry(paint.geometry, project.widthMM),
            data = describe(readGeometry(geometry));
          cells.push({
            ...data,
            key: `${o.id}:fallback:${i}`,
            objectId: o.id,
            ...
            color: colors.get(paint.swatchId),
            heightMM: paint.heightMM,
            zMM: bottomMM + (paint.zOffsetMM || 0),
            sourceIds: o.pathIds,
            conflict: false,
            choices: [paint.swatchId],
            conflictPaints: [],
            normalizedGeometry: paint.geometry,
          });
        }
