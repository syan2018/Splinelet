...
function validateArea(g, repair = false, warnings = []) {
  if (!g.isValid()) {
    ...
    g = union(
      polygonize([g.getBoundary()]).filter((c) =>
...
export function regionContext(project) {
  ...
  const calculate = (spec) => {
    ...
    if (spec.kind === 'path') {
      ...
      const q = [...a, ...(reverse ? b.slice().reverse() : b), a[0]];
      connections.push(
        {
          from: a.at(-1),
          to: q[a.length],
      ...
      for (let i = 0; i < cutters.length; i++) {
        ...
        for (const end of [0, -1]) {
          ...
          if (gap > 1e-7 && gap <= (spec.joinMM || 0)) {
            ...
            end === 0 ? q.unshift(beyond) : q.push(beyond);
            connections.push({
      ...
      const result = { cells, warnings, connections };
  ...
  const get = (id) => {
    ...
    try {
      ...
      if (spec.kind === 'split') {
        ...
        const hits = result.cells.filter((c) => c.contains(point(seed)));
        if (hits.length !== 1)
          throw Error('原选区定位点已不在唯一的面内，请重新选区');
        g = hits[0];
      }
      cache.set(id, { g, ...describe(g), warnings: result.warnings });
      return g;
    } catch (e) {
...
export function evaluateRegions(project) {
  ...
  return ctx.model.regions.map((r) => {
    try {
      ctx.get(r.id);
      const { g, ...result } = ctx.cache.get(r.id);
...
export function previewRegion(project, spec) {
  ...
  return {
    candidates: result.cells.map(describe),
