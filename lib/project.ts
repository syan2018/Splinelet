...
export function svg(project: Project) {
  return `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" width="${project.widthMM}mm" height="${fmt((project.widthMM * project.height) / project.width)}mm" viewBox="0 0 ${project.width} ${project.height}">\n<title>描迹 · 可编辑贝塞尔曲线</title>\n${project.paths
...
export function validateProject(v: any): Project {
  ...
  for (const p of v.paths) {
    if (
      ...
      !p.curves.every(
        (c: any) => Array.isArray(c) && c.length === 4 && c.every(point),
      ) ||
      !point(p.start) ||
