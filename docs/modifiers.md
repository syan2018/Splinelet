...
| 端点接边     | 曲线与边界 → 曲线     | 接边范围、允许的曲线组、端点开关     |
| 区域选择     | 区域 → 具名结果       | 局部坐标锚点、逐点唯一命中、组内合并 |
| 固定区域快照 | 保存的几何 → 区域     | 明确固化的修订与独立来源             |

现有算子的参数由卡片显示；参数或表达式驱动的字段不会被普通数字输入框覆盖。曲线参数的中心在界面中使用世界毫米坐标，命令转换到部件局部坐标，整体移动不改写局部来源。

## 添加、排序与删除

修改器卡片的「输入关联与结果快照」可查看本步骤最近一次已提交成功的输出，并显示其修订号。等待更新或失败时，旧快照仅供对照，不用于当前构造、属性赋值或导出。新工程/重开文件不会带入上一工程的历史快照。

对具名输入端口，可以明确选择新的来源，先预览、再确认替换；取消不修改工程，确认后一次撤销恢复。替换保留原坐标关系、参数及局部作用范围，不自动把失效的局部选择改为整个对象。本地坐标输入只列同部件来源，跨部件须明确使用世界坐标输入；缺失的必需输入可逐项补回。类型不匹配和新增依赖环会拒绝，预览期间修改工程会使候选失效。

「局部区域选择」列出当前选择条件及候选轮廓，可以手动重选、预览、取消或确认。重选保留该选择的颜色、高低与下游引用，已被另一选择占用的面不能被覆盖。「区域选择」修改器使用可编辑的局部毫米坐标锚点：每个点必须严格位于一个输入面内部，不作最近面搜索；同一组重复命中按声明的 union 策略合并。启用互斥检查的集合会拒绝不同属性组选中同一面。

「将区域快照保存为独立来源」创建一个断开连接的 Snapshot Source，可保存、撤销及显式接入。历史快照的坐标按其成功修订的世界变换转换到当前部件，避免部件移位后固化产生偏移。它不会自动接替失败步骤，也不会复制一份可写缓存作为作者状态。

「区域之后的求值链」显示外观、浮雕、打印位置、轮廓清理和实体的阶段状态、诊断及最近成功修订。外观与浮雕按部件缓存，放置保留跨部件层序与附着依赖；实体按 Part 隔离失败，存在阻断时整体导出仍会拒绝。

区域更新期间，当前作者文档中的数值参数、名称和启用状态仍可修改；连接、作用范围和结构操作继续使用各自的求值与结构检查。源手势进行中仍需先提交或取消，不能把旧区域结果作为新命令输入。
...
画布下方「派生样条」显示只读曲线阶段；源节点和控制柄仍是编辑入口。选中一个部件或仅属于它的源线、区域时才显示该部件的阶段选择，取消选择后恢复各部件的完整结果。默认只显示开关和「预览设置」，展开后可选择阶段、查看颜色说明及问题详情。橙点表示开放端点，笔画和辅助线允许开放；红点表示分叉。正常开放路径不作为错误显示，重复诊断合并；构面失败也能看到当前有效曲线。取消勾选可隐藏辅助线，关闭辅助预览不改变工程、撤销或导出。
