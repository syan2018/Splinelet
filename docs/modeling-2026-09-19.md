...
这里介绍从“工程 → 设置 → 工程高级构造”打开的“高级构造编辑器”。它管理整个工程的构面来源、体块和零件，不属于当前选区。日常描线、分区、填色与厚度在统一创作中完成，见 [统一创作指南](creation-2026-09-19.md)。需要两线围面、构面布尔、凹槽／贯穿或零件定义时，再打开这个命名明确的模态编辑器。它仍将源样条、派生面、实体特征分开保存；后两步不会增加或改动源贝塞尔节点。编辑器可在“面来源”和“体块／零件”间切换。
...
5. **导出**：从编辑器的“检查与导出当前零件”回到“工程 → 导出”，并带入当前零件；在这里检查实际尺寸、连通数量、非流形边和退化面，再导出实体。STL 只导出一个有效相连的闭合实体，坐标单位为毫米。Blender Python 包含独立隐藏集合中的所有源贝塞尔和最终实体；在 Blender 的 Scripting 中运行。精确源贝塞尔 SVG 在“工程 → 导出 → 源曲线 SVG”。高级编辑器中的“构造面数据 SVG”仅导出所选或全部旧构造配方的派生面轮廓，不含后续修改器和体块。

面和体块名称双击修改，单击只选择。画布 Shift / Ctrl 增减面选择；对象树 Shift 连选，Ctrl 增减。空白 / Esc 取消选择。侧栏可以拖动宽度。删除上游面或体块时，存在下游依赖会显示二次确认；整次操作一步撤销。

源线修改后会重算派生区域。无法匹配原输出身份时，该面明确失效；不会悄悄换成另一块区域。“重新指定来源 / 选区”在一次事务内迁移可唯一对应的赋值和下游引用，返回新的区域 ID；调用方应采用提交结果，不能继续使用旧区域 ID。改变预览参数或工程会使旧预览失效。
...
制造清理默认关闭；可预览开启 0.02 mm 清理以处理极小缝隙和零宽接触。清理会改变派生细节，需检查结果。导出会去除共面特征分界和浮点坍缩面，然后重新校验实际 STL 坐标。当前未实现圆角、斜面、自动最小壁厚分析或完整的自由曲面建模；保留分色实体的通用 3MF 由统一的“工程 → 导出”导出，高级编辑器只保留构造面数据 SVG。
...
`inspect_model` 返回完整区域 GeoJSON 和只读面板投影；它不是可写模型定义。`state.model` 提供简要状态。`select_regions`、`set_relief`、`set_model_options`、`create_part`、`select_part`、`get_relief_mesh`、`discard_region_preview` 经同一 V4 命令会话执行。`set_model_options` 可一次更新 `toleranceMM` 和 `manufacturingMM`，失败不会部分写入。`delete_model_object` 遇到可级联依赖时打开确认框；无法唯一处理的构造依赖会明确拒绝。`export_model` 返回内容或 STL 所需网格，不触发下载；原有 `export` 继续输出源线。

区域名称、显示、颜色与体块名称分别写入对应作者态；同一区域再次添加体块会建立独立引用贡献，保留来源几何。候选子集、区域重绑和删除均编制真实 Program，不把预览多边形存成第二份可编辑数据。保存、撤销、GUI 与 Agent 复用相同事务。
