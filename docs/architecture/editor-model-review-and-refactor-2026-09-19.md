...
以下是实施契约骨架，字段类型将在 R1 定稿；不是当前可读写格式。使用 ID 映射保存实体，数组只表示顺序或操作数集合，不能用数组位置当身份。
...
此表是实施顺序，不是本次已完成项。先用用户主路径核对对象关系和数据域，再建立内核与兼容证据，最后切换日常编辑。每个工作包分成可审阅提交；不以往旧 translatePaths 同步中心参数替代对象模型。

| 包                      | 依赖       | 具体改动与边界                                                                                                                | 完成条件                                                                                 |
| ----------------------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| R0 基线与契约           | 本评审     | 确定默认术语、对象关系和各结果域；走查 U01–U06 故事板与最小数据流；固化已有缺陷、成功/失败 fixture 和延迟基线                 | 简单闭合轮廓、部件内分区、重复纹样均能映射同一模型；无须增加默认技术层级；不改生产逻辑   |
| R1 V4 场景与几何核心    | R0         | schema/validator、Group/Shape、局部变换、稳定 Sketch IDs、Program 发布端口、外观/浮雕赋值与 EntityRef；结果类型独立于持久定义 | 空/曲线/区域/失败部件身份一致；归属、keepWorld 和元素身份不变量通过                      |
| R2 统一求值与迁移       | R1         | 分次交付数据域求值、旧算子逐项降级转换、身份与属性映射、V1–V3 importer/V4 codec；旧引擎只做对照                               | CurveSet 到 BodySet 各层可单独验证；多来源部件、逐输出浮雕和样例比较通过；无旧数据双写   |
| R3 事务与 API           | R2         | 统一 dispatcher、草案/历史/Worker；实现轮廓、分区、挖洞等任务命令编制真实 Program；API 调用同一命令                           | GUI/API 同结果；高级展开不改定义；基础链无需手工接线；迟到结果和取消不改工程             |
| R4 默认编辑与结果投影   | R3         | H 修改 pose；A/P 编辑局部源；区域选择和颜色/厚度使用对应结果/赋值；F 聚焦派生范围                                             | U01/U02 的简单主路径先通过，再验证移动、继续编辑、撤销/重开与纯派生部件                  |
| R5 编组与作品树         | R3、R4     | 默认部件/组树；展开线条/区域；按需构造详情；区分整理与源转移，保留失效对象入口                                                | 编组不改构造/制造；无默认四个技术目录或集合管理器；U03/U06 通过                          |
| R6 基准、关系与连接     | R2、R4     | 基准画布 gizmo；共享参数；沿轴/重合/柄连续性；保存关系；通用 Join 算子；派生元素回写映射                                      | 1/8 开放母线完整验收；改中心/方向/允许的重复参数仍接合；尖角保留；Alt 不暗删永久关系     |
| R7 浮雕、制造与导出切换 | R2、R3、R6 | 切换外观/浮雕赋值、放置/制造解析与 Part 映射；显示/导出读同一 snapshot 的指定阶段                                             | 默认成品可直接导出；源曲线与成品阶段不混用；颜色、层数、切削、材料体积及实体检查通过     |
| R8 默认启用与旧写入退役 | R0–R7      | 默认打开即迁移内存；新文件 V4；旧文档只读导入；清除旧 GUI/Agent 写路径和推导；更新所有当前指南                                | 全部验收矩阵及双端检查通过；新文档往返稳定；旧文件未被自动覆盖；生产运行只有一个编辑模型 |

依赖主链表示完整交付门槛：`R0 → R1 → R2 → R3 → R4 → R5/R6 → R7 → R8`。R2 先以一个闭合轮廓和一个重复纹样验证各数据域，再逐个迁移旧来源，避免一次性重写所有引擎。其核心接口稳定后可提前在隔离 V4 原型中串联 R3/R4 的简单主路径；若普通任务需要理解端口或阶段，先修正边界再扩大迁移。R5/R6 可在边界稳定后分开开发，不并行改同一根组件。

### 9.1 模块落点与退役映射

以下是建议路径，开始对应包前确定最终命名；目录与导入迁移遵循 AGENTS.md 的映射、批量修复和搜索规则。

| 当前模块                                       | 目标职责/建议落点                                                      | 过渡后处理                                                          |
| ---------------------------------------------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------- |
| project.ts 的混合模型                          | `src/lib/document/types.ts`、`schema.mjs`                              | 当前类型转成 legacy import 类型；下载/旧源导出不混入核心 schema     |
| creation-schema.mjs                            | `src/lib/document/migrate-v3.mjs` + 纯 V4 validator                    | 推导/修复仅留在导入，运行时删除 normalize-as-read                   |
| selection.mjs 的对象位移                       | `src/lib/scene/transforms.mjs`                                         | source editor 只保留几何元素编辑                                    |
| node-edit/continuity/spline-edit               | `src/lib/geometry/sketch.mjs`、纯节点算法                              | 改用稳定 ID，旧索引 API 留适配器                                    |
| curve/modifier/region/partition engines        | `src/lib/construction/registry.mjs`、`evaluate.mjs`、各纯算子          | 数学内核复用；删除并行隐式构造路径                                  |
| surface-lineage 与分散 styles                  | `src/lib/construction/provenance.mjs`、统一 attributes                 | 保留失效协议，不保留多处权威写入                                    |
| creation-engine 中的颜色/厚度/体块混合         | `src/lib/relief/` 的赋值解析与 ReliefSet；制造服务产出 PlacedReliefSet | 删除 cells 再包装为旧可编辑 features 的回路；实体内核只消费明确结果 |
| 根 transact、creationCommand、model API 写操作 | `src/lib/editing/commands.mjs`、`transaction.mjs`                      | React 只发命令，兼容 API 不直写旧数组                               |
| use-creation-selection                         | `src/hooks/use-editor-selection.ts` + 纯 EntityRef helper              | 保留语义选择思路，去掉 paths 作为对象操作代理                       |
| creation-workspace 作品树与 properties         | 编辑器任务投影、作品树与按需构造详情                                   | 保留简单主路径，内部结果类型不自动成为用户层级                      |
| print-stack / export                           | 制造求值与导出适配                                                     | 保留整数层/实体内核，输入统一 V4 snapshot                           |
| project-format / persistence / platform        | 版本分派、资源 codec、保存队列                                         | 延续平台边界与文件安全，不重写无关底层                              |

### 9.2 每个包的合并纪律

- 提交说明列出新权威数据、移除的旧权威、迁移映射和覆盖的验收项；不能只报告新增字段或 UI 按钮。
- 先运行覆盖面的单测/隔离浏览器，再按仓库要求运行 `pnpm check:all`。影响 Manifold Worker/wasm 的改动必须包含生产构建。
- 浏览器使用独立测试端口与新 context，使用提交的最小 fixture，不读取或替换用户当前工程。
- 开发期间新旧引擎可作离线对照；编辑器在任一时刻只使用一种文档模型。中间阶段不对日常工程启用不完整的 V4 writer。
- 自动转换只提交到内存草案；保存是明确操作。回滚代码不能声称可以无损读写 V4→V3；要保留原始 V3 文件和 migration report。
- 每个涉及界面的包走查对应 U 项；“技术上可通过高级面板完成”不能替代默认流程验收。不得通过新增全局工作区、模式或术语解决内部归属不清的问题。
