...
3. **整理集合 `Collection` 是引用关系，用于旧信息保留与源线列表编组。** 它没有坐标系，不隐式合并、移动或改变构造。普通整理统一使用“编组”；不同时向普通用户推出“组 / 集合 / 标签”三套组织入口。
...
原源线列表的“编组”使用纯 PathRef 集合；明确移入时从其他纯路径集合移出，混合实体集合保持不变。源线组的显隐是一次对当前成员 Path 的批量命令，不是集合继承属性。高级操作产生的重叠成员在只读展示中保留全部 groupIds，只有唯一成员归属才提供旧 groupId；不得任意选择一个组掩盖重叠。源线排序只改 Path.order，不能修改 Node.parentId、pose 或 Program 输入顺序。
...
- 绘制首点时，零边 `Path.startVertexId` 引用一个真实 Vertex；首段创建后删除该字段，起点由首条有向 edge use 决定。这样保留首次落点的显示、保存和撤销，也不制造零长度 Edge 或重复保存坐标。单节点尚无曲线/区域输出；来源缺失仍可诊断并保存。
...
- `Path.order` 是跨 Sketch 的源列表显示顺序；`Collection.order` 保留整理分组的显示顺序，组内引用按 `members` 数组顺序保存。二者为可选有限数，缺省按 0 再以稳定 ID 排序，兼容早期 V4 文档。旧工程导入明确写入原路径与分组顺序，新线条追加到源列表末尾。显示排序不改 Shape 所有权、Node 层级顺序、构造输入或制造/Z；Collection 不成为新的默认用户概念。
...
在已有部件上追加独立闭合轮廓时，新源通过 Source → Fill 构面，与旧发布区域通过 region-collect 汇总；已有曲线与新曲线通过 curve-collect 汇总。汇总保留输入身份、区域引用和 Join 接合关系，不创建新副本身份、不重写旧算子或赋值。重复身份输入应阻断；这些结构性连接由语义命令管理，默认修改器列表不展示 Source、Fill 或汇总节点。

已有线条暂停参与某个构面输入时，使用 `curve-filter` 的 `excludedPaths: PathRef[]` 表达该输入的成员排除；原 Source、Fill 和发布端口保持不变。恢复时移除同一排除项，因此多轮廓 even-odd 的内圈仍回到原来的孔洞，而不是新建独立填充区域。筛选不生成副本身份、不改变源的可编辑性；失效引用阻断并保留待修复定义。`set-region-path-membership` 明确指定路径、区域算子与是否参与，隔离共享筛选的其他消费者；同一 Fill 输入服务多个不同区域用途时要求先明确拆分。该连接不作为默认修改器或重复曲线预览展示，普通用户仍通过原来的线条用途操作。

恒等局部的成员筛选仍属于普通构面管理。继续绘制只扩展输入，不新建独立 Fill，也不覆盖现有 Fill 参数；暂停成员改成开放路径后仍留在筛选前的 Source 中，避免普通闭合路径枚举将其移除而破坏恢复引用。变换或其他实际派生阶段仍通过高级分支规则处理。

原用途操作通过 `set-path-roles` 原子提交：优先暂停/恢复已有输入成员；新的边界加入唯一的普通私有 Fill 来源，在已组合的构造中建立独立分支；新挖洞和分区使用当前区域目标创建规范算子。用途控制的 Difference/Partition 可显式采用 `emptyInput: 'passthrough'`，当切割输入为空时保留上游结果和身份，不解析暂未参与的旧目标；输入恢复后重新校验原 scope/输出契约。未指定此策略的既有算子保持原语义，真正失败的切割输入仍阻断。默认界面明确显示未参与的参考线；复杂派生来源或多用途不猜测成单一用途，须编辑对应构造。

分区/挖孔从第一点开始保存真实的未发布构造分支。终端 Partition/Boolean 的 `authoring:{phase:'drawing'}` 只区分尚在绘制与普通禁用修改器；准确目标保存在 scope，来源保存在 inputs。绘制期间保留旧区域发布端口，未完成分支不影响已有作品；完成时校验目标和旧发布端口仍有效，原子启用、发布及迁移赋值。不能将禁用算子视为旁路，也不能在重开时依据临时界面角色猜测重建构造。
...
分区的可选 `endpointJoin` 保存接边容差、明确禁用的 Path 端点和已接受的分区线批次；使用 V4 来源身份。求值时根据当前底面与原始分区线重新寻找连接，延伸段只存在于求值结果和接边预览中，不能写入隐藏 Path/Edge/Vertex。后添加的分区线可以连接此前批次，但不能反过来吸走旧端点、改变已接受的分区关系。未启用该策略时不自动补边；真正改变区域拓扑仍须验证输出契约。
...
2026-09-20 实施补充：实体网格输出遵守 Manifold 显式拓扑合并表；坐标重合不等于拓扑同一，不得按坐标自动焊接不同顶点。浮点精度修复发生在派生实体阶段，修复后仍执行异常边、零面积面、体积与材料分区检查，不改写规范源数据。

## 原界面接线的作者态补充 · 2026-09-20

双端默认入口只建立 V4 host，原工作区仅消费只读投影与规范命令。旧 Project 的 React 写状态、旧模型 Worker 和旧保存/恢复队列不再提供回退。旧格式解释只发生在打开时的单向导入。

高级面板的区域名及显示开关以可选 `Document.regionPresentations.overrides` 记录，每条为 `{id,target:OutputRef,name?,visible?}`。它们是编辑器展示作者态，不写几何，也不改变浮雕或制造贡献。完整 OutputRef 匹配；同一部件的另一个区域不受影响；失效记录保留并在高级投影中报告，不按面积或顺序猜测绑定。已有文件可省略该表，首次写入时创建；复制按完整引用重映射，含展示绑定的源转移/重构纳入保护检查。

浮雕覆盖记录可带独立 `name`，修改厚度等值不会丢失名称。删除单区域浮雕需要能屏蔽部件默认贡献，因此覆盖记录允许 `suppressed:true`，且要求 `value.enabled:false`：它表示明确删除，区别于仍列在面板内的禁用定义。再次启用或首次建立浮雕清除该标记。源曲线和区域输出都保留；保存与撤销复用同一 Document。

同一区域上的额外浮雕贡献通过独立 Shape 的 `region-reference` 消费一个明确选区，以 `world-result` 引用来源。新增定义拥有自身的厚度、位置、名称与制造归属，不复制求值多边形或恢复旧 features 表。引用算子的可选 `scope` 使用完整 OutputRef；不指定时仍引用整个输入 RegionSet，保持既有构造兼容。

制造清理由可选 `manufacturing.cleanupRadiusMM` 定义，缺省为零，必须为非负有限数。它只参与 PlacedReliefSet 到 BodySet 的截面清理，源 Sketch、RegionSet 及其身份不变；主实体和材料分区必须复用同一清理截面。旧工程的 `model.manufacturingMM` 单向迁移到此字段，不能因求值成本而忽略。

高级区域预览持有已编制的 Program 文档及候选 OutputRef；确认复用该文档，不从显示多边形反建来源。候选子集通过明确的区域引用发布，重绑定在同一事务内迁移作者态；无法唯一映射的消费者必须报告，不能按候选序号猜测。区域删除保留原始线条，并检查赋值、依附与外部构造引用。
