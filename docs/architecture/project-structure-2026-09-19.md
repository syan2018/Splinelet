...
| `components/studio/`                                      | 共享应用主体、工作区组合与应用状态              |
| `components/shell/`                                       | 窗口控制等宿主界面                              |
| `components/{creation,modeling,source-editor,shared,ui}/` | 各工作区与通用界面                              |
| `hooks/`                                                  | 可复用 React 状态逻辑                           |
| `lib/platform/`                                           | Tauri 桥接、浏览器下载及平台分派                |
| `lib/source-editor/`                                      | 选择、节点编辑、连续性、连接与画布数学          |
| `lib/persistence/`                                        | IndexedDB 恢复草稿与文件写入队列                |
| `lib/` 其他模块                                           | 领域模型、几何构造、实体、导出和模型 Worker     |
| `types/`                                                  | Vite 资源与 Worker 导入的环境类型声明           |
...
`app/page.tsx` 与 `desktop/main.tsx` 都引用 `components/studio/studio-app.tsx`。客户端边界放在共享组件上，Web 路由不承担应用实现，桌面入口不导入 Web 路由组件。

`lib/platform/index.mjs` 提供跨平台下载分派，并导出桌面能力；`desktop.mjs` 封装 Tauri API，`browser.mjs` 负责浏览器下载。工程绑定、打开、保存与恢复的编排仍留在共享应用中。继续提取时应按行为边界拆分，不复制两套保存状态，不改变 `window.traceStudio.call`。
...
| 原路径                                                                       | 当前路径                                                      |
| ---------------------------------------------------------------------------- | ------------------------------------------------------------- |
| `app/page.tsx` 中的应用主体                                                  | `components/studio/studio-app.tsx`；原路径保留薄路由          |
| `components/desktop-window-controls.tsx`                                     | `components/shell/desktop-window-controls.tsx`                |
| `lib/desktop-runtime.mjs`                                                    | `lib/platform/{index,desktop,browser}.mjs`                    |
| `public/{canvas-gestures,connect,continuity,extend,node-edit,selection}.mjs` | `lib/source-editor/` 下同名文件                               |
| `public/persistence.mjs`                                                     | `lib/persistence/workspace.mjs`                               |
| `public/creation-pick.mjs`                                                   | `lib/creation-pick.mjs`                                       |
| `components/creation/model-worker.d.ts`                                      | `types/worker.d.ts`                                           |
| `lib/vite-assets.d.ts`                                                       | `types/vite-assets.d.ts`                                      |
