...
日期：2026-09-26。状态：设计决策已收敛，产品尚未实施。本文替代同日初稿；当前实现仍见[构造链](construction-pipeline.md)，故障事实见[删点复审](../qa/spline-edit-failure-2026-09-26.md)，交付顺序见[工作包](../../tasks/region-identity-redesign-2026-09-26/README.md)。
...
示意结构（领域边界已选定，字段实现须经过原型和 schema 审查）：
...
只为被作者引用的目标保存定义。临时显示的每个面无需分配持久 UUID。UI 命中使用 `{evaluationToken, cellId}`，只能在同一次求值中有效；命令根据当前已验证结果构造选择器后才建立持久引用。过期 cellId 不能用于涂色、厚度或重新选择。
...
- 同一路径的连续区间合并：新拟合曲线覆盖合并后的有向域，并明确采用新的参数化。只保证逻辑域连续，不伪称几何完全等价；区域条件在新几何上重新求值。跨不同逻辑用途不能如此合并。
- 闭环跨接缝的区间按周期域表达；不得把原点重新吸附到某个现存顶点。删除显式语义锚点仍是删除，不以保留参数域掩盖它。

该数据量由当前 use/定义数量决定，不保存无限增长的删点历史。上述公式是新源命令的规范语义；尚需用现有所有编辑命令及跨接缝用例验证，不能直接从 V4 当前 Edge 数组推断用户历史。
...
交点锚点包含有序的两个 `(use, parameterDomain)` 分支。自交允许两分支引用同一个 use，但其域必须可区分且互不重叠；域来自持久源级基准及命令映射，不来自当前交点附近临时生成的窗口。域内多个交点、无法分类的接触/重合或交点越过声明域边界，均不能通过选择剩余 face 自动消歧。
...
当前 `sampleCubic → GeometryNoder → union → Polygonizer` 将用途、方向和参数逐步丢掉，再由 `contourSignatures` 按中点距离找回来源。新设计必须替换这个信息往返过程。
...
保留现有几何核的可用部分。当前 JSTS `NodedSegmentString` 可以携带 data，split 会继承 data；但现有 `GeometryNoder` 封装传入的是 null。应建立一个明确的 tagged noding/arrangement 适配层，而不是另造几套相似度匹配。
...
JSTS 可继续处理采样多段线的 noding、布尔、buffer 和验证；没有来源保证的返回值不能直接作为新选择器依据。半边构建、重合来源集合、孔/非流形处理和生成边界归因是必须实现与验证的基础设施，不是已存在能力。CGAL 的带历史 arrangement 明确区分输入曲线和由其诱导的图边，提供了这一职责划分的参考；这不等于决定引入 CGAL/WASM。[CGAL 官方说明](https://doc.cgal.org/latest/Arrangement_on_surface_2/index.html)
...
Worker 保留内部来源图，显示消息只包含所需几何、短定义引用、状态和诊断。完整来源/实体快照是按需检查接口；同一依赖 DAG 的上游信息不重复嵌套到每个输出引用中。消息由哪个边界复制、哪边持有可变内存要明确，不能简单删 clone 破坏不可变性。
...
新 `.spl` schema 使用新版本（拟为 5），不能静默修改 V4 引用含义。迁移只在文件打开边界执行一次，新运行时只有一个作者模型。
...
迁移结果是完整新 Document 或独立的未完成导入报告，不能产生一半 V4、一半新协议的可写工程。迁移未通过的文件保持原字节和文件绑定保护，使用冻结旧适配器提供只读检查/诊断；允许复制原文件，禁止自动保存覆盖为新格式。用户修复或重新定义目标后，须全量迁移校验成功才能进入新可写会话。这个检查模式不属于新运行时的求值 fallback，也不能作为正常旧工程无法继续编辑的免责手段：正常 V4 表达能力缺失就是发布阻断项。
...
底层 tagged noding 能传 data，只证明接口可用；完整 arrangement、生成边界来源和选择器表达能力仍必须做原型及反例验收。在这些门禁通过前，可以独立实施源交互分离，但不能将新命名方案当成已证明完整的产品能力。
