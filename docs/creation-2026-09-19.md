...
选择与同步逻辑集中在 `hooks/use-creation-selection.ts`；画布坐标、缩放和定位在 `lib/source-editor/canvas-gestures.mjs`；构面封口/补边面板在 `components/creation/creation-connections.tsx`。源编辑器只接收选择的路径投影；portal 面层明确转交主画布的平移手势。几何命令不依赖显示方式。
