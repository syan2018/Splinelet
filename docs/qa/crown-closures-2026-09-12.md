...
> Status: historical QA snapshot. Referenced local output artifacts are not stored in this repository.
...
from the workspace copy. The one-off repair script used for this snapshot is no longer retained; it repaired the four
...
Pure regressions at the time: the now-retired `test-crown-closures.mjs`, `test-model.mjs`,
