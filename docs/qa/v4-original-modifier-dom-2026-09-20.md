...
create-support 已通过原 creation_base 预览/确认接入：选中部件外形使用规范 region-outline/boolean/offset 引用，厚度和 placement 单独赋值；继承唯一制造零件，普通模式顶面依附、打印模式插入最底层。预备句柄保持只读身份，取消无写入，伪造/重复/失效预览拒绝。单元核对源几何和构造不变，普通叠放 2 mm、打印 3 × 0.2 = 0.6 mm 的层基准以及一次撤销。
