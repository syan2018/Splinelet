...
导入后，在大纲展开导入组、选中笔画部件，在「构造」中展开「SVG 笔画」卡片，调整「笔画宽度」（mm）。宽度沿中心线两侧扩张，和「颜色与高低」中的浮雕厚度独立；修改可撤销、重做并随工程保存。一个 Stroke 步骤使用统一宽度，SVG 的压力数据不会导入；变宽笔迹若已转为填充轮廓，会保留轮廓形状，但没有中心线宽度参数。
...
修改器后的完整轮廓默认可见，不依赖选择。对于只发布 Fill 区域的部件，预览菜单默认使用“构面输入 · 完整轮廓”；“最终曲线”仍只代表显式曲线输出。变换的显示预览不创建模型 preview，松手才发布一次工程修改；A 的节点编辑仍保留实时源预览。详见 [编辑管线](architecture/editor-interaction-pipeline-2026-09-21.md)。
