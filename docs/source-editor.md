...
| 变换 G / R / S | 面或线选择所属部件；修饰键增减部件    | 拖动选中部件的全部源线，保持内部相对位置       | 空白不变换对象       |

节点模式编辑当前一条路径中的多个节点；Ctrl+A 全选其节点后可整体移动这条路径。选择模式不移动形状。G / R / S 切换到变换，将当前区域或线条选择提升为所属部件；未选部件第一次按下只选中，松手后再次按住才可拖动；拖动已选部件保留多部件选择，整体移动全部来源。按住 Shift / Ctrl / Cmd 点击只增减部件，拖动开始后按 Shift 限制水平或垂直方向。隐藏路径可以在路径树中管理，但不会被画布框选。
...
从文件菜单选择「导入 SVG／笔迹…」，或拖入 SVG（不超过 5 MB）。设置宽度、中心位置、浮雕厚度，以及可选的贴附部件（按导入中心所在区域的表面高度贴附，避开孔洞和区域边界）；中心以工程中心为原点，单位毫米，Y 向上。导入建立一个场景组，已有工程保留，整批导入一次撤销。

支持 path 的直线、三次／二次贝塞尔和圆弧，以及矩形、圆、椭圆、多边形、折线与线段；嵌套变换和实色填充、孔洞保留。圆弧以最多 45° 的三次贝塞尔段近似。描边成为可编辑中心线与 Stroke 构面，采用圆头／圆角；其他线帽／接头会提示外观差异。Blender Grease Pencil 笔迹请先导出 SVG；不直接读取 .blend 文件。

导入后，在大纲展开导入组、选中笔画部件，在「选区 → 构造」中展开「SVG 笔画」卡片，调整「笔画宽度」（mm）。宽度沿中心线两侧扩张，和「选区 → 浮雕」中的浮雕厚度独立；修改可撤销、重做并随工程保存。一个 Stroke 步骤使用统一宽度，SVG 的压力数据不会导入；变宽笔迹若已转为填充轮廓，会保留轮廓形状，但没有中心线宽度参数。

文字须先转路径；图片、use 实例、渐变、裁剪、蒙版、滤镜及虚线等不支持的外观会拒绝并提示先展开。导入过程中不执行 SVG 脚本或加载外部图片。

## 整组变换

点击左侧「变换」默认进入移动，并展开带图标和快捷键的三种模式；G / R / S 直接切换移动、旋转、等比缩放，H 不再绑定工具。右侧「变换」面板同步显示当前选区、模式和对应操作提示，换选对象时保持打开。可输入 X / Y 位移毫米、旋转角度或缩放百分比后应用；切换模式本身不修改工程。输入数值、重命名、弹窗、中文输入法组合输入或拖动过程中不触发模式切换，Ctrl+S 仍保存工程。

画布统一使用 [Moveable](https://github.com/daybrush/moveable) 变换控件：拖动已选对象或选框移动、四角等比缩放、顶部圆柄旋转。所有手柄同时可见，G / R / S 与侧栏选择突出当前模式；拖动对象本身始终移动。控件大小保持屏幕像素尺寸，随画布缩放、平移和选区旋转更新位置。

选框采用当前求值轮廓；只有源线的对象使用源曲线范围。单个部件或场景组的选框跟随其世界旋转方向，多选独立根对象采用世界轴对齐框。旋转围绕中心，缩放固定拖拽角的对角，按住 Alt 开始拖动则固定中心；数值旋转和缩放均使用中心。选中场景组时包含所有后代，多选父子不会重复变换。

拖动时，选框、源线、派生轮廓与填充面同步显示；画布显示毫米位移、旋转角度或缩放百分比与尺寸。规范文档在松手前保持不变，期间拒绝 API 写入与导出。

拖动中 Shift 将移动限制为水平／垂直、旋转限制到 15°、缩放限制到 10% 步进。Esc、失焦或指针取消恢复原位置；松手只提交一次，可 Ctrl+Z 撤销。缩放只支持正比例等比缩放，保留浮雕厚度与制造层高，同时缩放曲线与构造中的空间尺寸。锁定后代阻止整组操作；缩放遇到跨选区空间关系或构造输入会拒绝，需把关联对象一起选中。
...
`load_project` 接受 `{expectedRevision,project: 旧版工程对象}` 或 `{expectedRevision,base64: 完整工程容器}`，二者只能提供一个，可附带显示用 `filename`。会话复用文件打开管线：V5 容器严格校验文档与资源；V4 或旧对象完整迁移为 V5。成功后重置历史和选区，保持无文件绑定，转换后的工程标为待保存。可以将 `export({format:'json'})` 返回的 `base64` 传回此入口；不要传只读展示对象或缺少资源的裸文档 JSON。失败保留当前工程；绘制计算、保存或拖动期间拒绝替换。
