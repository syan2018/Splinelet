...
`agent/v4-browser.mjs` 提供 API 5.0 的浏览器/WebMCP 挂载和串行本地 companion 轮询；卸载时撤销注册并停止后续排队任务。`scripts/agent-server.mjs` 接受对应动作名。当前仅模块验证，原 Studio 的 API 4.1 入口尚未切换，不能把传输适配算作 GUI/API 全旅程完成。

后续原面板命令适配已补齐：设置厚度同时启用候选浮雕；部件参与导出与 Part 归属写 Shape 配置；修改器启停写对应算子；已启用打印分层后的层高、新增、重命名、排序和空层删除使用资源命令。部件视图从明确制造配置读取 printable/partId，不能把未上色、空或求值失败误当作禁止导出。尚未支持的开启分层、整部件层归属、对象放置及复杂修改器属性继续拒绝，不能以部分字段更新冒充旧动作完成。
...
2026-09-19：实现原子提交、固定基线预览、单调 revision、换工程 epoch、会话签发的一次性异步草案及撤销/重做。主代理审阅后补上预览期间拒绝外部提交、伪造草案拒绝、通知异常隔离、命令重入拒绝与选择意图传递；`test-v4-transactions.mjs` 已通过。尚未接入 GUI/Worker/文件保存，不能据此签收完整 A13。
