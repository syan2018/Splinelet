...
旧 path recipe 的自动封口用 `path.params.closure: 'straight'` 明确表达：对一个连续 Curve 的实时首末端点构造直线边界，支持 even-odd 与显式 repair，不新增可写 Path/Edge 或隐藏标记。输入内部断缝、多条曲线或额外 Junction 明确拒绝；未声明 closure 的 Path/Fill 继续要求显式闭合拓扑。封口来源随求值传递，不能成为第二份原始数据。

2026-09-19 动态几何补充：布尔、内缩和描边的一个语义结果可用 MultiPolygon 保留多个离散面，不能按连通块重复签发同一个 OutputRef。只有显式分区建立独立区域身份。`between` 的显式 `repair: true` 复用纯几何奇偶填充修复；源线变化后仍重新求值，修复结果不写回源。`test-v4-region-core.mjs` 已覆盖布尔切断、空操作数、内缩断连和自交带修复后的源点修改及身份保持。
