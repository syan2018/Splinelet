...
候选页面 `?editor=v4` 使用 `window.traceStudioV4.call(action,args)` 暴露 API 5.0；能力以 `capabilities.get` 的实际结果为准。它尚未接替旧页面的 `window.traceStudio`、HTTP bridge 或 WebMCP。写操作统一要求 `expectedRevision`，求值/导出要求明确的 epoch/revision；旧数组索引写入被拒绝。
