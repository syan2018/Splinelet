...
建档：2026-09-19。状态：**实现完成，按用户指定真实工程范围验收通过**，代码提交 `5c49d0c`。本目录管理分发、依赖、交付和验收；架构事实以[编辑模型方案](../../docs/architecture/editor-model-review-and-refactor-2026-09-19.md)为唯一来源。原第 10 节的 U01–U06、A01–A23 保留为完整设计矩阵，本次最终运行验收采用下述用户明确校正的范围，不宣称每条历史 UI 旅程均重新执行。

## 当前收敛状态 · 2026-09-20

用户明确调整执行顺序：**先切断旧运行路径，再在唯一 V4 管线上恢复行为**。默认入口已经创建 V4 宿主；原 Studio 的可写 React 工程、旧保存/恢复队列、旧历史更新和两个工作区的旧 Model Worker 入口已移除。旧工程解码仅保留为打开时的单向迁移，不是编辑回退。

本轮已验证原界面布局、Sandrone 创作编辑/撤销/保存重开、高级面板 69 区域/69 体块及有效三维结果；本机 Tauri 验证内置 Sandrone 和 gold 的打开、节点编辑、撤销、V4 副本保存重开且原件不变。证据与边界见[验收索引](acceptance-2026-09-19.md)。

高级恢复包已接入规范区域预览、候选子集提交、区域删除与引用检查、重绑定、独立浮雕贡献、区域/体块属性、制造清理、零件归属以及线性修改器排序和删除。布尔与分区生成独立派生部件，保留输入部件；预览提交复用候选定义，不从结果多边形反建源。原界面的树整理、源转移、对象放置、打印分层及补边也已接线，普通闭合/开放线条与空部件整理可一次撤销。制造清理的真实样例超时已修复；准确检查结果、安全限制与原生保存边界统一记录在[收尾快照](../../docs/qa/v4-default-cutover-2026-09-20.md)，不沿用旧候选界面 PASS。
...
- 用户同时授权整理原前端已有的交互混乱和管线边界问题：优先收敛重复状态、命令写入、求值与生命周期；正常布局和操作习惯仍保留。实际行为修正逐项给出问题、变化和回归证据，不能据此扩大为整套界面重做。
...
- 合同版本：[v1.3](contracts-2026-09-19.md)，Operator.authoring 标记覆盖未完成分区、挖洞及高级部件追加轮廓；真实 C03 联合链与新增消费者检查的阶段证据见验收索引及工作包。
...
T21/T22 的原生验收按用户确认在本机进行，不要求独立账户或 VM。以专用进程打开内置 Sandrone，完成编辑、撤销、临时副本保存与重开；gold emblem 是补充样例。单实例转交等补充检查不得把测试工程发送到用户正在运行的实例。`bundle.active=false`，不生成安装包或注册关联。
