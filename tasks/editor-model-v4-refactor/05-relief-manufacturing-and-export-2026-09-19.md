...
2026-09-20 实体精度与原输出面板：V4 BodySet 导出使用 Manifold 的 mergeFromVert/mergeToVert 重建拓扑，不因坐标相同而合并不同顶点；再按已有 STL 精度处理与严格网格检查生成数据。内置 Sandrone 与金色徽章均生成单个有效实体、完整材料分区和有效 STL/3MF，规范文档不变。原成品检查/通用 3MF 已接 Worker 与 BodySet；未以此签收完整 Bambu 实际切片、全部高级建模面板或默认入口。
