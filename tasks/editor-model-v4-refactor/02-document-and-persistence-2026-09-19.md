...
2026-09-19 后续源语义核对已通过：

- 两份工程的 47/48 个 smooth 节点映射为 Path.handleModes，保留双向拖柄与对侧长度。
- 跨 Shape 来源改为唯一可写 Path 加 curve-reference；修改共享外轮廓后 10 个消费者均更新，依赖图无环。
- 路径组保留精确且有序的 PathRef 成员；不扩大为整个 Shape，不改变变换父子关系。
- 开放路径封口改为 Path 的显式 straight closure；端点修改驱动封口线更新，没有隐藏可写标记或冻结轮廓。

**T18 仍待原工作区完整打开、编辑、保存重开验收，不因模块测试通过而签收。** 真正多所有者声明目前结构化拒绝；跨 owner 的多曲线 between/split 尚无 collect 表达时也明确拒绝，不能复制源作为兜底。两份指定工程不触发这两种拒绝。
