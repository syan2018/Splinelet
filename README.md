...
- **参考图叠放**：从画布「显示选项 → 管理参考图」打开图层面板，保留基准底图，添加多张独立参考图；支持平移、等比缩放、旋转、翻转、排序、透明度、锁定与撤销，随 `.spl` 一起保存。使用方式见[参考图指南](docs/reference-images-2026-09-22.md)。
- **辅助描线**：跟随深色线条或颜色边缘拟合三次贝塞尔。相邻两个落点之间只生成一段曲线，不插入额外锚点；控制柄可手工调整。
- **导入 SVG／笔迹**：从文件菜单或拖放导入 SVG，保留可编辑贝塞尔；填充和孔洞构面，描边成为有宽度的浮雕笔迹，可设置尺寸和贴附表面。导入后在「构造 → SVG 笔画」调整笔画宽度，与浮雕厚度独立。Blender 笔迹先导出 SVG，文字先转路径。
- **对象变换**：左侧「变换」展开移动、旋转、等比缩放，分别用 G / R / S 直接切换；画布共用 Moveable 选框、四角缩放柄与旋转柄，轮廓和实时数值同步，每次应用可一步撤销。
- **编辑样条**：节点移动、删除、连续性设置，头尾续画、端点合并、批量选择与拖拽，配合撤销和重做。
- **大纲删除**：大纲聚焦时按 Delete / Backspace 删除选中的部件、场景组（含后代）或源线；支持批量删除与撤销，锁定项会阻止整批删除。
- **组织区域**：按部件管理轮廓、分区线、挖洞线和参考线；局部区域分别设置颜色和厚度，项目色卡统一管理配色。
- **组合构造**：曲线可镜像、阵列、显式连接后构面；面修改器支持分区、布尔差／并／交和轮廓偏移。Join 表单提供可辨识的边端和实例预览；上游关系失效时暂停受影响的输出链，独立部件继续预览。
...
**Ctrl+S** 首次选择文件后绑定保存，之后每次按下 Ctrl+S 或点击顶栏「保存」才写回同一文件；**Ctrl+Shift+S** 另存为。编辑后的恢复草稿会自动保存，供刷新或意外关闭后恢复，但草稿不是文件备份的替代品。不支持文件访问 API 的浏览器会下载 `.spl`。格式细节见 [Splinelet 工程格式](docs/project-format.md)。

普通查看会自动显示修改器补全后的轮廓；只发布 Fill 区域的部件显示其明确的构面输入，无需先选中。独立对象整体移动可复用不受姿态影响的局部平面结果，世界坐标依赖变化仍重新计算。适用范围及交互与求值边界见 [编辑管线](docs/architecture/editor-interaction-pipeline-2026-09-21.md)。

## 常用操作

左侧「变换」默认进入移动；G / R / S 直接进入移动、旋转、等比缩放，H 不再绑定工具。变换采用先选后拖：首次按下未选部件只选中，拖动已选对象或选框移动，四角等比缩放，顶部圆柄围绕中心旋转。三种操作共用 [Moveable](https://github.com/daybrush/moveable) 控件，选框、轮廓与实时数值同步；缩放默认固定对角，按住 Alt 以中心缩放，Shift 约束移动方向或按 15° / 10% 步进。拖动期间只显示预览，Esc 取消，松手提交一次可撤销的修改。模式按钮与属性面板同步，换选对象时保持变换面板打开；在输入框、重命名、弹窗或拖动中不会触发模式快捷键。
...
详细节点、路径、合并、续画和兼容 API 操作见 [源线编辑器手册](docs/source-editor.md)。属性栏竖排分为“工具 / 工程 / 选区”，当前按钮向右与内容区连成一个面板；没有选择时不显示“选区”。部件和区域按“形状、颜色、浮雕、叠放”分开显示，单个部件还提供构造、承托和输出页；源线只显示源线页，场景组只显示层级页。需要两线围面、构面布尔、凹槽／贯穿或零件定义时，从“工程 → 设置 → 工程高级构造”打开高级构造编辑器。选中一个部件或仅属于它的源线、区域时可选择其派生曲线预览阶段；取消选择后恢复完整结果。
...
- [选区属性贡献架构](docs/architecture/property-contributions-2026-09-26.md)
...
平面和立体画布支持[派生样条预览](docs/modifiers.md#派生样条预览)：在「预览设置」中切换源线、镜像和阵列阶段，拖动源节点即时查看结果，构面失败时仍显示当前曲线与开放端点。正常开放笔画不显示未闭合警告；说明和问题详情默认收起，可取消「派生样条」勾选隐藏辅助线。API 可通过 `creation_inspect.curvePreviews` 读取相同阶段与诊断。
