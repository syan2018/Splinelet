...
- P 描线：第一次点击创建锚点，移动显示白色虚线预览，下次点击确认沿边缘的贝塞尔路径。
...
多尺度局部暗谷与 Sobel 梯度形成像素代价。A* 在两锚点扩张的边界框中搜索，并加入偏离引导线的代价；像素路径平滑后用端点切线约束的最小二乘三次贝塞尔拟合，误差超限递归拆分。使用 Worker 避免阻塞 UI；预览最多保留一个待计算请求。手动模式直接连接，之后编辑控制柄。
