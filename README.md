...
**Ctrl+S** 首次选择文件后绑定保存，之后每次按下 Ctrl+S 或点击“保存工程”才写回同一文件；**Ctrl+Shift+S** 另存为。编辑后的浏览器草稿会自动保存，供刷新或意外关闭后恢复，但始终明确为未保存工程文件。不支持文件访问 API 时，保存会下载工程 JSON；浏览器缓存不是文件备份的替代品。
