...
- **对象变换**：左侧「变换」展开移动、旋转、等比缩放，分别用 G / R / S 直接切换；属性面板显示当前选区、数值与拖动提示，每次应用可一步撤销。
...
左侧「变换」默认进入移动；G / R / S 直接进入移动、旋转、等比缩放，H 不再绑定工具。变换采用先选后拖：首次按下未选部件只选中，再次拖动才变换；拖动期间使用显示预览，松手提交一次可撤销的修改。模式按钮与属性面板同步，换选对象时保持变换面板打开；在输入框、重命名、弹窗或拖动中不会触发模式快捷键。
