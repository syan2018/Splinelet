import { validateSlicerTemplate } from './bambu-3mf.mjs';
...
export function validateModel(m) {
  if (m?.slicerTemplate) validateSlicerTemplate(m.slicerTemplate);
  if (
    !m ||
    m.version !== 1 ||
  ...
  for (const f of m.features) {
    if (
      f.heightLayers !== undefined &&
      (!Number.isInteger(f.heightLayers) ||
        f.heightLayers < 1 ||
        f.heightLayers > 100000)
    )
      throw Error('体块厚度必须是整数打印层数');
    if (
