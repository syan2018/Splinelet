...
function scaleGeometry(g, factor) {
  if (!g.coordinates)
    return {
      ...g,
...
export function compileCreation(project) {
  ...
  const manufacturingInfo = (cell, object) => ({
    objectId: object.id,
    objectName: object.name,
    cellKey: cell.key,
    regionName: cell.name || object.name,
    swatchName:
      scene.creation.swatches.find((s) => s.id === cell.swatchId)?.name ||
      cell.color,
    printLayerId: cell.printLayerId,
    printLayerName: scene.creation.printStack?.layers.find(
      (l) => l.id === cell.printLayerId,
    )?.name,
    order: scene.cells.indexOf(cell),
  });
  for (const cell of scene.cells.filter((c) => c.featureId)) {
    const feature = p.model.features.find((f) => f.id === cell.featureId);
    if (feature) {
      feature.manufacturing = manufacturingInfo(
        cell,
        objects.get(cell.objectId),
      );
      feature.color = cell.color;
    }
  }
  ...
  for (const c of scene.cells.filter(
    ...
    p.model.features.push({
      ...
      color: c.color,
      enabled: true,
      manufacturing: manufacturingInfo(c, o),
    });
  }
  return p;
}
