...
import { normalizePrintObjects, validatePrintStack } from './print-stack.mjs';
...
export function dividerIdsFromTopology(topologyKey) {
  try {
    ...
    if (Array.isArray(roles))
      return new Set(
        roles.filter(([, role]) => role === 'divider').map(([id]) => id),
      );
  } catch {}
  return new Set();
}
