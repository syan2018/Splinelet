...
export function resolveSketch(document, sketchId, context = {}) {
  ...
  for (const path of Object.values(sketch.paths)) {
    ...
    for (let index = 0; index < uses.length; index++) {
      ...
      resolved.push({
        key: `${path.id}:${edge.id}:${occurrence}`,
        cubic: use.reversed ? [...cubic].reverse() : cubic,
        startKey: use.reversed ? edge.endVertexId : edge.startVertexId,
        endKey: use.reversed ? edge.startVertexId : edge.endVertexId,
    ...
    if (resolved.length !== uses.length) continue;
    if (resolved.length)
      curves.push({
        key: path.id,
